...
class ConcatDataset(torchConcatDataset):
    ...
    def load_anno(self, idx):
        """为ConcatDataset添加load_anno方法，使其与mosaicdetection.py中的mixup方法兼容"""
        if idx < 0:
            if -idx > len(self):
                raise ValueError(
                    "absolute value of index should not exceed dataset length"
        ...
        if dataset_idx == 0:
            ...
            sample_idx = idx - self.cumulative_sizes[dataset_idx - 1]
        # 委托给具体的子数据集来加载标注
        return self.datasets[dataset_idx].load_anno(sample_idx)
    ...
    def read_img(self, index, use_cache=True):
        ...
        # 委托给具体的子数据集来读取图像，传递use_cache参数
        return self.datasets[dataset_idx].read_img(sample_idx, use_cache=use_cache)
